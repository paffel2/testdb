...
import HelpFunction
    ( readByteStringToInt,
      ...
      readByteStringToDay,
      toQuery )
import Logger ( logError, logInfo, Handle, logDebug )
...
getNewsByIdFromDb hLogger pool (Just news'_id) = catch ( do
        ...
        if Prelude.null rows then do
            ...
            return $ Right $ Prelude.head rows )$ \e -> do
                                            ...
                                            return $ Left "Database error"
        where
            q = toQuery $ BC.concat ["with image_arr as (select array_agg(image_id) from news_images where news_id = ",BC.pack $ show news'_id,"), ",
...
getDraftByIdFromDb hLogger pool token_lifetime token' draft_id = do
    ...
    case ch_author of
        ...
        Right author_id -> catch (do
            rows <- queryWithPool pool q [author_id]
            if Prelude.null rows then do
