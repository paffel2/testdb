...
createCategoryOnDb hLogger pool token_lifetime token' (Just category'_name) (Just maternal_name) = catch (do
    ch <- checkAdmin hLogger pool token_lifetime token'
    case ch of
        (False, bs) -> return $ Left bs
        (True,_) -> do
            if maternal_name == "" then
                createWithoutMaternal
                else
                    createWithMaternal) $ \e -> do
                                let err = E.decodeUtf8 $ sqlErrorMsg e
...
deleteCategoryFromDb hLogger pool token_lifetime token (Just categoryName) = catch (do
    ch <- checkAdmin hLogger pool token_lifetime token
    case ch of
        (False, bs) -> return $ Left bs
        (True,_) -> do
...
editCategoryOnDb hLogger pool token_lifetime token (Just old_name) (Just new'_name) (Just "") = catch (do
    ch <- checkAdmin hLogger pool token_lifetime token
    case ch of
        (False, bs) -> return $ Left bs
        (True,_) -> do
            ...
            if n > 0 then do
                ...
                return $ Left "Category not exist") $ \e -> do
                                ...
                                case errStateInt of
                                    23505 -> return $ Left "Category already exist"
                                    _ -> return $ Left "Database error"
editCategoryOnDb hLogger pool token_lifetime token (Just old_name) (Just "") (Just new'_maternal) = catch (do
    ch <- checkAdmin hLogger pool token_lifetime token
    case ch of
        (False, bs) -> return $ Left bs
        (True,_) -> do
            m_id <- queryWithPool pool "select category_id from categories where category_name = ?" [new'_maternal] :: IO [Only Int]
            n <- executeWithPool pool "update categories set maternal_category = ? where category_name = ?" (fromOnly $ myHead m_id, old_name)
            if n > 0 then do
                logInfo hLogger $ T.concat ["Category ", old_name, " edited"]
                return $ Right "Category edited"
            else do
...
editCategoryOnDb hLogger pool token_lifetime token (Just old_name) (Just new'_name) (Just new'_maternal) = catch( do
    ch <- checkAdmin hLogger pool token_lifetime token
    case ch of
        (False, bs) -> return $ Left bs
        (True,_) -> do 
            ...
            n <- executeWithPool pool q (new'_name, fromOnly $ myHead m_id, old_name)
            if n > 0 then do
                logInfo hLogger $ T.concat ["Category ", old_name, " edited"]
                return $ Right "Category edited"
            else do
