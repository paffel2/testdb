...
newsMethodBlock hLogger path pathElems req
    | pathElemC == 1 = do
    ...
    | pathElemC == 4 =
        case last pathElems of
            "add_comment" -> do
                ...
                result <- addCommentByNewsId hLogger req news_id
                case result of
                    Left bs -> return $ responseBadRequest bs
...
addCommentByNewsId hLogger req news_id = do
    ...
    case comment of
        ...
        Just bs' -> do
                addCommentToDb hLogger (E.decodeUtf8 $ fromMaybe "" token) news_id (E.decodeUtf8 $ fromMaybe "" comment)
